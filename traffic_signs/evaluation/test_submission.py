...
week = 1  # Week number
window_evaluation = 0  # Whether to perform or not window evaluation: 0 for week 1, 1 for week 2
...
for method in methods:
    ...
    if result_files_num != test_files_num:
        print('Method {} : {} result files found but there are {} test files'.format(method, result_files_num,
                                                                                     test_files_num))
    ...
    for ii in range(len(result_files)):
        ...
        [localPixelTP, localPixelFP, localPixelFN, localPixelTN] = evalf.performance_accumulation_pixel(pixelCandidates,
                                                                                                        pixelAnnotation)
        ...
        if window_evaluation == 1:
            ...
            [localWindowTP, localWindowFN, localWindowFP] = evalf.performance_accumulation_window(windowCandidates,
                                                                                                  windowAnnotations)
    ...
    [pixelPrecision, pixelAccuracy, pixelSpecificity, pixelSensitivity] = evalf.performance_evaluation_pixel(pixelTP,
                                                                                                             pixelFP,
                                                                                                             pixelFN,
                                                                                                             pixelTN)
    ...
    if window_evaluation == 1:
        [windowPrecision, windowSensitivity, windowAccuracy] = evalf.performance_evaluation_window(windowTP, windowFN,
                                                                                                   windowFP)  # (Needed after Week 3)
        windowF1 = 0

        print('Team {:02d} window, method {} : {:.2f}, {:.2f}, {:.2f}\n'.format(team, method, windowPrecision,
                                                                                windowSensitivity, windowF1))
